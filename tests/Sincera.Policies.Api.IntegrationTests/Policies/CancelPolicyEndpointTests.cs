...
public class CancelPolicyEndpointTests : IClassFixture<ApiFactory>
{
    ...
    [Fact]
    public async Task POST_cancel_returns_400_when_reason_missing()
    {
        var request = new { effectiveCancellationDate = "2026-06-01", reason = "" };
        var response = await _client.PostAsJsonAsync("/api/policies/P-1/cancel", request);
    ...
    public async Task POST_cancel_already_cancelled_returns_409()
    {
        ...
        var first = await _client.PostAsJsonAsync($"/api/policies/{policy.Id.Value}/cancel", request);
        Assert.Equal(HttpStatusCode.OK, first.StatusCode);

        var second = await _client.PostAsJsonAsync($"/api/policies/{policy.Id.Value}/cancel", request);
